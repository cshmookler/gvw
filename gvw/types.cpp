...
glfw_shared_init_hints::glfw_shared_init_hints(int Joystick_Hat_Buttons)
    : glfw_hints({ { { GLFW_JOYSTICK_HAT_BUTTONS, Joystick_Hat_Buttons } } },
                 { {} })
{
}
...
glfw_macos_init_hints::glfw_macos_init_hints(int Cocoa_CHDIR_Resources,
                                             int Cocoa_Menubar)
    : glfw_hints({ { { GLFW_COCOA_CHDIR_RESOURCES, Cocoa_CHDIR_Resources },
...
glfw_general_hints::glfw_general_hints(int Resizable,
    ...
    : glfw_hints({ { { GLFW_RESIZABLE, Resizable },
...
glfw_framebuffer_hints::glfw_framebuffer_hints(int Red_Bits,
                                               ...
                                               int SRGB_Capable,
                                               int Double_Buffer)
    : glfw_hints({ { { GLFW_RED_BITS, Red_Bits },
...
glfw_monitor_hints::glfw_monitor_hints(int Refresh_Rate)
    : glfw_hints({ { { GLFW_REFRESH_RATE, Refresh_Rate } } }, { {} })
{
}
...
glfw_input_event_callbacks::glfw_input_event_callbacks(
    std::optional<GLFWkeyfun> KeyCallback,
    std::optional<GLFWcharfun> CharacterCallback,
    std::optional<GLFWcursorposfun> CursorPositionCallback,
    std::optional<GLFWcursorenterfun> CursorEnterCallback,
    std::optional<GLFWmousebuttonfun> MouseButtonCallback,
    std::optional<GLFWscrollfun> ScrollCallback,
    std::optional<GLFWdropfun> FileDropCallback)
    : keyCallback(KeyCallback.value_or(window_t::KeyCallback))
    , characterCallback(CharacterCallback.value_or(window_t::CharacterCallback))
    , cursorPositionCallback(
          CursorPositionCallback.value_or(window_t::CursorPositionCallback))
    , cursorEnterCallback(
          CursorEnterCallback.value_or(window_t::CursorEnterCallback))
    , mouseButtonCallback(
          MouseButtonCallback.value_or(window_t::MouseButtonCallback))
    , scrollCallback(ScrollCallback.value_or(window_t::ScrollCallback))
    , fileDropCallback(FileDropCallback.value_or(window_t::FileDropCallback))
{
}

glfw_window_event_callbacks::glfw_window_event_callbacks(
    std::optional<GLFWwindowclosefun> CloseCallback,
    std::optional<GLFWwindowsizefun> SizeCallback,
    std::optional<GLFWframebuffersizefun> FramebufferSizeCallback,
    std::optional<GLFWwindowcontentscalefun> ContentScaleCallback,
    std::optional<GLFWwindowposfun> PositionCallback,
    std::optional<GLFWwindowiconifyfun> IconifyCallback,
    std::optional<GLFWwindowmaximizefun> MaximizeCallback,
    std::optional<GLFWwindowfocusfun> FocusCallback,
    std::optional<GLFWwindowrefreshfun> RefreshCallback)
    : closeCallback(CloseCallback.value_or(window_t::CloseCallback))
    , sizeCallback(SizeCallback.value_or(window_t::SizeCallback))
    , framebufferSizeCallback(
          FramebufferSizeCallback.value_or(window_t::FramebufferSizeCallback))
    , contentScaleCallback(
          ContentScaleCallback.value_or(window_t::ContentScaleCallback))
    , positionCallback(PositionCallback.value_or(window_t::PositionCallback))
    , iconifyCallback(IconifyCallback.value_or(window_t::IconifyCallback))
    , maximizeCallback(MaximizeCallback.value_or(window_t::MaximizeCallback))
    , focusCallback(FocusCallback.value_or(window_t::FocusCallback))
    , refreshCallback(RefreshCallback.value_or(window_t::RefreshCallback))
{
}
